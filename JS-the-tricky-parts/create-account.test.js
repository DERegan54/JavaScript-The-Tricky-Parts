...
describe("createAccount", function() {
  ...
  it("accepts an optional starting balance as a second argument", function() {
    expect(bigAccount.checkBalance("4321")).toBe("Your current balance is $10000.");
  });
  ...
  describe("checkBalance", function() {
    ...
    it("returns the balance if the pin is correct", function() {
      expect(account.checkBalance("1234")).toBe("Your current balance is $0.");
    });
  });
  ...
  describe("deposit", function() {
    ...
    it("adds the amount to the account balance", function() {
      expect(account.deposit("1234", 100)).toBe(
        "You successfully deposited $100. Your current balance is $100."
      );
      expect(account.deposit("1234", 200)).toBe(
        "You successfully deposited $200. Your current balance is $300."
      );
      expect(account.checkBalance("1234")).toBe("Your current balance is $300.");
    });
  });
  ...
  describe("withdraw", function() {
    ...
    it("subtracts the amount from the account balance", function() {
      expect(bigAccount.withdraw("4321", 250)).toBe(
        "You successfully withdrew $250. Your current balance is $9750."
      );
      expect(bigAccount.withdraw("4321", 1000)).toBe(
        "You successfully withdrew $1000. Your current balance is $8750."
      );
    });
  ...
  describe("changePin", function() {
    ...
    it("changes the PIN with a success message if the PIN matches", function() {
      ...
      expect(account.checkBalance("5678")).toBe("Your current balance is $0.");
    });
  });
});
